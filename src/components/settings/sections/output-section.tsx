    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 rounded-md border p-3">
        ...
        <Input
          ...
          min={1}
          max={16}
          value={String(draft.ingestConcurrency)}
          onChange={(e) => {
            const v = Number(e.target.value)
            if (Number.isFinite(v) && v >= 1 && v <= 16) setDraft("ingestConcurrency", v)
          }}
        />
        <p className="text-xs text-muted-foreground">
      ...
      <div className="space-y-2 rounded-md border p-3">
        <div className="flex items-center justify-between">
          <Label>{t("settings.sections.output.ingestConcurrencyScheduleEnabled")}</Label>
          <button
            type="button"
      ...
      <div className="space-y-2 rounded-md border p-3">
        <div className="flex items-center justify-between">
          <div>
            <Label>{t("settings.sections.output.speculativeScan", "Speculative scan")}</Label>
          </div>
          <button
            type="button"
            role="switch"
          ...
          >
            <span
              className={`pointer-events-none block h-4 w-4 rounded-full bg-white shadow transition-transform ${
                draft.speculativeScanEnabled ? "translate-x-4" : "translate-x-0"
              }`}
            />
          </button>
        </div>
        <p className="text-xs text-muted-foreground">
          {t("settings.sections.output.speculativeScanHint", "Pre-check pending files against the ingest cache to skip unchanged sources.")}
        </p>
      </div>
    </div>
  )
